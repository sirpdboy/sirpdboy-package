...
command:value('/etc/init.d/samba restart &&/etc/init.d/samba4 restart',translate("E.重启共享"))
command:value('/etc/init.d/network restart',translate("F.重启网络"))
command:value('ifdown wan  && sleep 5&& ifup wan',translate("G.重启WAN"))
command:value('killall -q pppd && sleep 1 && ifup wan', translate("H.重新拨号"))
