...
if running then
        state_msg = "<b><font color=\"green\">" .. translate("启动运行") .. "</font></b>"
else
        state_msg = "<b><font color=\"red\">" .. translate("没有运行") .. "</font></b>"
end

a = Map("webrestriction", translate("访问限制"), translate("【名单控制】的简化版，仅能使用MAC黑名单或者MAC白名单模式控制列表中的客户端联网，而无其他功能。").. button .. "<br/><br/>" .. translate("运行状态").. " : "  .. state_msg .. "<br />")
e = a:section(TypedSection, "basic", translate(""))
e.anonymous = true
t = e:option(Flag, "enable", translate("开启（<font color=\"red\">总开关</font>）") , translate(""))
t.rmempty = false
...
if e then
  io.popen("/etc/init.d/webrestriction restart")
end
